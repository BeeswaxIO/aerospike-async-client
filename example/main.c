...
static bool complete_phase_7(int return_value, ev2citrusleaf_bin* bins,
		int n_bins, void* pv_udata);
static bool start_phase_8(uint32_t generation);
...
const phase_start_fn PHASE_START_FUNCTIONS[] = {
	...
	start_phase_7,			// increment an integer bin and return the result
	start_phase_8,			// delete the record
	NULL
};
...
const phase_complete_fn PHASE_COMPLETE_FUNCTIONS[] = {
	...
	complete_phase_7,		// verify increment result
...
start_phase_5(uint32_t generation)
{
	...
	ev2citrusleaf_object_init_int(&bin.object, 1234);
...
//------------------------------------------------
// Increment an integer bin and return the result.
//
static bool
start_phase_7(uint32_t generation)
{
	ev2citrusleaf_operation ops[2];

	// Increment bin A by 1.
	strcpy(ops[0].bin_name, "test-bin-A");
	ops[0].op = CL_OP_ADD;
	ev2citrusleaf_object_init_int(&ops[0].object, 1);

	// Read bin A.
	strcpy(ops[1].bin_name, "test-bin-A");
	ops[1].op = CL_OP_READ;
	ev2citrusleaf_object_set_null(&ops[1].object);

	if (0 != ev2citrusleaf_operate(
			g_p_cluster,					// cluster
			(char*)g_config.p_namespace,	// namespace
			(char*)g_config.p_set,			// set name
			&g_key,							// key of record to write
			ops,							// ops (array) to apply
			2,								// two ops for this transaction
			&g_write_parameters,			// write parameters
			g_config.timeout_msec,			// transaction timeout
			client_cb,						// callback for this transaction
			NULL,							// "user data" - nothing for us
			g_p_event_base)) {				// event base for this transaction
		LOG("ERROR: fail operate() to increment integer-valued bin");
		return false;
	}
...
//------------------------------------------------
// Verify the result of the increment operation.
//
static bool
complete_phase_7(int return_value, ev2citrusleaf_bin* bins, int n_bins,
		void* pv_udata)
{
	if (return_value != EV2CITRUSLEAF_OK) {
		LOG("ERROR: client callback return_value %d", return_value);
	...
	else if (strcmp(bins[0].bin_name, "test-bin-A") != 0) {
		LOG("ERROR: unexpected bin name %s", bins[0].bin_name);
	}
	else if (bins[0].object.type != CL_INT) {
		LOG("ERROR: unexpected bin type %d", bins[0].object.type);
	}
	else if (bins[0].object.u.i64 != 1235) {
		LOG("ERROR: unexpected value %ld", bins[0].object.u.i64);
	}
	else {
		valid = true;
	}
...
//
static bool
start_phase_8(uint32_t generation)
{
