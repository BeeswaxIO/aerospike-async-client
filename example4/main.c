...
put(int b, int k)
{
	...
	// Always the same size blob-type value, with a byte of k embedded.
	char value[g_config.value_size];

	*(uint8_t*)value = (uint8_t)k;
...
validate_data(int b, int k, ev2citrusleaf_bin* bins, int n_bins)
{
	...
	else if (*(uint8_t*)bins[0].object.u.blob != (uint8_t)k) {
		LOG("ERROR: base %2d, key %d, got unexpected data value %u", b, k,
				*(uint8_t*)bins[0].object.u.blob);
	}
